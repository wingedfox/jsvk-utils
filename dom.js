 ...
 *  @param {Window} win window object to look the stylesheet, if null current one will be used
 *  @return {Boolean} true if any stylesheet for the given name found
 *  @scope public
 */
DOM.hasStylesheet = function (sname, win) {
    return Boolean(DOM.getStylesheet(sname, win));
}
/**
 *  Checks whether the stylesheet has been attached to the document
 *
 ...
 *  @scope public
 */
DOM.getStylesheet = function (sname, win) {
  if (!sname) {
      return false;
  }
  if (!/\.css$/i.test(sname)) {
  ...
  var h  = win.document.getElementsByTagName('head')[0]
     ,sr = new RegExp('<link[^>]+?href\\s*=\\s*["\']?(([^>]+?/|)'+sname+'[^"\'\\s]*)[^>]*>','i')
  
  return (h && h.innerHTML.match(sr)[1]) || "";
}
/**
 *  Removes given stylesheet from the document
 *
 ...
 *  @scope public
 */
DOM.removeStylesheet = function (sname, win) {
  if (!sname) {
      return false;
  }
  if (!win || !win.document) {
...
/**
 *  Performs check for the stylesheet existance
 *
 *
 */
DOM.addStylesheet = function () {
}
/**
